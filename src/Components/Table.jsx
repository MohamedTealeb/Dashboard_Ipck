...
import {
  ...
  TextField, Stack, MenuItem, FormControl, InputLabel, Select, Grid
} from '@mui/material';
    ...
    <Paper sx={{ marginTop: '80px', width: '100%', overflow: 'hidden' }}>
      <Stack direction="row" justifyContent="flex-end" p={2}>
        <Button
          ...
          sx={{ fontSize: { xs: '0.8rem', sm: '1rem' }, padding: { xs: '6px 12px', sm: '8px 16px' } }}
        >
          Add User
        </Button>
      </Stack>
              ...
              <TableBody>
                {rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>
                      <Button onClick={() => handleOpenFormDialog('edit', row)} sx={{ padding: '4px' }}>
                        <FaUserEdit size={18} />
                      </Button>
                      <Button color="error" onClick={() => handleOpenDeleteDialog(row.id)} sx={{ padding: '4px' }}>
                        <MdDelete size={20} />
                      </Button>
                    </TableCell>
                  </TableRow>
